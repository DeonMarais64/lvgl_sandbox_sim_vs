/**
 * @file sndbx_pge_lv_test_theme_1.c
 *
 */
 ...
 *      INCLUDES
 *********************/
#include "sndbx_pge_lv_test_theme_1.h"
...
#define PAGE_NAME "lv Test #ff0000 Theme 1# - Managed"
#else
#define PAGE_NAME "LittlevGL Test #ff0000 Theme 1# - Managed version"
#endif
...
typedef struct {
	...
	lv_obj_t * line;
	lv_point_t line_p[2];
	lv_calendar_date_t cal_highlighted_days[2];
} mem_t;

/**********************
 *  STATIC PROTOTYPES
 **********************/
static void create_tab1(lv_obj_t * parent, void * p_mem);
static void create_tab2(lv_obj_t * parent, void * p_mem);
static void create_tab3(lv_obj_t * parent, void * p_mem);
...
void sndbx_pge_lv_test_theme_1_test(void)
{
 ...
 */
static void test_theme_1(lv_obj_t *parent, lv_theme_t * th, void * p_mem)
{
    lv_theme_set_current(th);
	/*If `LV_THEME_LIVE_UPDATE  1`..     */
	/* ..`th` is not used directly ..    */
	/* .. so get the real theme after set*/
    //th = lv_theme_get_current();    
    lv_obj_t * cnt = lv_cont_create(parent, NULL);
//    lv_disp_load_scr(scr);

    lv_obj_t * tv = lv_tabview_create(cnt, NULL);
//    lv_obj_set_size(tv, lv_disp_get_hor_res(NULL), lv_disp_get_ver_res(NULL));
    lv_obj_set_size(tv, lv_obj_get_width(parent), lv_obj_get_height(parent));
    ...
    create_tab1(tab1, p_mem);
    create_tab2(tab2, p_mem);
    create_tab3(tab3, p_mem);
}
...
static void create_cb(lv_obj_t *parent, void *p_mem, const void *prms)
{
	MEM_INIT(mem, p_mem);
	mem->parent = parent;
	test_theme_1(parent,NULL,p_mem);
}
...
static void create_tab1(lv_obj_t * parent, void * p_mem)
{
    ...
    lv_style_copy(&mem->h_style, &lv_style_transp);
    mem->h_style.body.padding.inner = LV_DPI / 10;
    mem->h_style.body.padding.left = LV_DPI / 4;
    mem->h_style.body.padding.right = LV_DPI / 4;
    mem->h_style.body.padding.top = LV_DPI / 10;
    mem->h_style.body.padding.bottom = LV_DPI / 10;

    lv_obj_t * h = lv_cont_create(parent, NULL);
    lv_obj_set_style(h, &mem->h_style);
    ...
    mem->lbl_prim = lv_label_create(h, NULL);
    lv_label_set_text(mem->lbl_prim, "Primary");
    lv_obj_set_style(mem->lbl_prim, th->style.label.prim);

    mem->lbl_sec = lv_label_create(h, NULL);
    lv_label_set_text(mem->lbl_sec, "Secondary");
    lv_obj_set_style(mem->lbl_sec, th->style.label.sec);

    mem->lbl_hint = lv_label_create(h, NULL);
    lv_label_set_text(mem->lbl_hint, "Hint");
    lv_obj_set_style(mem->lbl_hint, th->style.label.hint);

    static const char * const btnm_str[] = {"1", "2", "3", LV_SYMBOL_OK, LV_SYMBOL_CLOSE, ""};
    ...
    mem->line = lv_line_create(h, NULL);
    mem->line_p[0].x = 0;
    mem->line_p[0].y = 0;
    mem->line_p[1].x = lv_disp_get_hor_res(NULL) / 5;
    mem->line_p[1].y = 0;

    lv_line_set_points(mem->line, mem->line_p, 2);
    lv_line_set_style(mem->line, LV_LINE_STYLE_MAIN, th->style.line.decor);
...
static void create_tab3(lv_obj_t * parent, void * p_mem)
{
    ...
    mem->cal_highlighted_days[0].day = 5;
    mem->cal_highlighted_days[0].month = 5;
    mem->cal_highlighted_days[0].year = 2018;

    mem->cal_highlighted_days[1].day = 8;
    mem->cal_highlighted_days[1].month = 5;
    mem->cal_highlighted_days[1].year = 2018;

    lv_calendar_set_highlighted_dates(cal, mem->cal_highlighted_days, 2);
    lv_calendar_set_today_date(cal, &mem->cal_highlighted_days[0]);
    lv_calendar_set_showed_date(cal, &mem->cal_highlighted_days[0]);

    /*Create a Message box*/
    static const char * const mbox_btn_map[] = {" ", "Got it!", " ", ""};
    lv_obj_t * mbox = lv_mbox_create(parent, NULL);
    lv_mbox_set_text(mbox, "Click on the window or the page to bring it to the foreground");
    lv_mbox_add_btns(mbox, (const char **)mbox_btn_map);
