 ...
 *      DEFINES
 *********************/
#define PAGE_NAME "Sandbox Template"
 ...
 *  GLOBAL DESCRIPTORS
 **********************/
const sndbx_pge_dsc_t sndbx_pge_template_dscs[SNDBX_PGE_TEMPLATE_NB_DSCS] = {
	{
		.name = PAGE_NAME" 1",
...
#define NB_DSCS (sizeof(sndbx_pge_template_dscs)/sizeof(sndbx_pge_template_dscs[0]))

/**********************
 *  STATIC VARIABLES
 **********************/

/* Forward declaration of parameters, if required */
static const struct sndbx_pge_template_prms _prms[NB_DSCS];

/* Different flavours of the same page, if required */
static const sndbx_pge_t pages[NB_DSCS] = {
	{.dsc = &sndbx_pge_template_dscs[0], .prms = &_prms[0] },
	{.dsc = &sndbx_pge_template_dscs[1], .prms = &_prms[1] },
	{.dsc = &sndbx_pge_template_dscs[2], .prms = &_prms[2] },
};
#define NB_PAGES (sizeof(pages)/sizeof(pages[0]))

/* Links between pages, if required */
static const sndbx_pge_template_prms_t _prms[NB_DSCS] = {
	{.links.left =      NULL, .links.right = &pages[1], },
	{.links.left = &pages[0], .links.right = &pages[2], },
	{.links.left = &pages[1], .links.right =      NULL, },
};
 ...
 */
void sndbx_pge_template_test( void )
{
...
static void create_cb( lv_obj_t * parent, void * p_mem, const void * prms )
{
	...
	const sndbx_pge_template_prms_t * pge_prms = (const sndbx_pge_template_prms_t *)prms;
